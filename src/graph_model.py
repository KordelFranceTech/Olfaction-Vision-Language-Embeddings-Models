...
from src.constants import AROMA_VEC_LENGTH, IMG_DIM
...
# Note that this graph is a generic neural network to help with edge export
class GraphAssociator(nn.Module):
    ...
    def forward(self, vision_embed, olfact_embed):
        x = torch.cat([vision_embed, olfact_embed], dim=-1)
        return self.fc(x)
...
if __name__ == "__main__":
    ...
    graph_model = GraphAssociator().to(DEVICE)
